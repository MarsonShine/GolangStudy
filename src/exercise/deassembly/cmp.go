...
每个函数还需要注释，在其参数、结果和本地堆栈帧中给出实时指针的位置。对于没有指针结果、本地堆栈帧或没有函数调用的汇编函数，唯一的要求是在同一个包的 Go 源文件中为该函数定义一个 Go 原型。汇编函数的名称不能包含包名组件（例如，包 syscall 中的函数 Syscall 在其 TEXT 指令中应使用"中点符号"名称`·Syscall`，而不是等效名称 syscall·Syscall）。对于更复杂的情况，需要显式注释。这些注释使用标准#include funcdata.h 文件中定义的伪指令。
