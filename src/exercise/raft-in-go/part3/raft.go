...
持久化状态
因为要实现高可用，当节点出现断开连接或宕机之后重连要继续正常运行，这就需要我们把关键数据和状态持久化：
currentTerm: 此时服务器观察到的最新的任期
votedFor: 该服务器在最近一次投票中投给了哪个节点id（peerId）
log: raft总的日志条目

因为节点会发生网络分区，崩溃等场景，所以接收到的命令至少会有一次，所以各个节点为了防止重复逻辑，在发送命令的时候需要带有唯一标识，从而可以实现幂等性。
有了持久化能力，节点崩溃恢复就提供了支持。
*/
...
func (cm *ConsensusModule) restoreFromStorage() {
	if termData, found := cm.storage.Get("currentTerm"); found {
		...
		log.Fatal("currentTerm not found in stroage")
	}
